  ...
  if (view === "rights") {
    return (
      <div>
        ...
        <h2 className="mb-4 text-[18px] font-black text-gray-900">{t("views.patientRights")}</h2>
          ...
          {UNIVERSAL_RIGHTS.map((right) => (
            <div key={right.id} className="rounded-2xl bg-white shadow-sm" style={{ border: "1px solid rgba(0,0,0,0.04)" }}>
              ...
              >
                ...
                <span className="flex-1 text-[14px] font-bold text-gray-800">{t(`rights.${right.id}.title`)}</span>
              ...
              {expandedRight === right.id && (
                <div className="border-t border-gray-50 px-4 pb-4 pt-3 animate-fade-in">
                  <p className="text-[13px] leading-relaxed text-gray-600">{t(`rights.${right.id}.description`)}</p>
                </div>
              )}
            </div>
          ))}
  ...
  if (view === "discrimination") {
    return (
      <div>
        ...
        <h2 className="mb-2 text-[18px] font-black text-gray-900">{t("views.discrimination")}</h2>
        <p className="mb-4 text-[13px] text-gray-500">{t("views.discriminationDesc")}</p>
        <div className="space-y-3">
          {DISCRIMINATION_SCENARIOS.map((scenario) => {
            const steps = t.raw(`scenarios.${scenario.id}.whatToDo`) as string[];
            return (
              <div key={scenario.id} className="rounded-2xl bg-white shadow-sm" style={{ border: "1px solid rgba(0,0,0,0.04)" }}>
                ...
                >
                  ...
                  <span className="flex-1 text-[14px] font-bold text-gray-800">{t(`scenarios.${scenario.id}.situation`)}</span>
                ...
                {expandedScenario === scenario.id && (
                  <div className="border-t border-gray-50 px-4 pb-4 pt-3 space-y-3 animate-fade-in">
                    <div className="rounded-xl bg-blue-50 p-3 border border-blue-100">
                      <p className="text-[11px] font-black uppercase tracking-wider text-blue-600 mb-1">{t("labels.sayThis")}</p>
                      <p className="text-[13px] font-bold italic text-blue-800">{t(`scenarios.${scenario.id}.whatToSay`)}</p>
                    </div>
                    <div>
                      <p className="text-[11px] font-black uppercase tracking-wider text-gray-400 mb-2">{t("labels.thenDo")}</p>
                      {steps.map((step, i) => (
  ...
  if (view === "contacts") {
    return (
      <div>
        ...
        <h2 className="mb-4 text-[18px] font-black text-gray-900">{t("views.contacts")}</h2>
        <div className="space-y-3">
          {LEGAL_CONTACTS.map((contact) => (
            <div key={contact.id} className="rounded-2xl bg-white p-4 shadow-sm" style={{ border: "1px solid rgba(0,0,0,0.04)" }}>
              <div className="mb-3 flex items-center gap-2">
                <span className="text-xl">{contact.flag}</span>
                <span className="text-[15px] font-black text-gray-900">{t(`contacts.${contact.id}.country`)}</span>
              </div>
              <div className="space-y-2">
                <div className="rounded-xl bg-gray-50 p-2.5">
                  <p className="text-[11px] font-bold text-gray-400">{t("labels.patientOmbudsman")}</p>
                  <p className="text-[13px] font-semibold text-gray-800">{t(`contacts.${contact.id}.ombudsman`)}</p>
                ...
                <div className="rounded-xl bg-gray-50 p-2.5">
                  <p className="text-[11px] font-bold text-gray-400">{t("labels.antiDiscrimination")}</p>
                  <p className="text-[13px] font-semibold text-gray-800">{t(`contacts.${contact.id}.antiDiscrimination`)}</p>
                ...
                {t.has(`contacts.${contact.id}.romaRightsOrg`) && (
                  <div className="rounded-xl bg-red-50 p-2.5 border border-red-100">
                    <p className="text-[11px] font-bold text-red-400">{t("labels.romaRightsOrg")}</p>
                    <p className="text-[13px] font-bold text-red-800">{t(`contacts.${contact.id}.romaRightsOrg`)}</p>
                  </div>
                )}
              </div>
            </div>
    ...
    <div>
      ...
      <div className="space-y-3">
        ...
        >
          ...
          <div>
            <span className="text-[15px] font-bold text-gray-900">{t("menu.patientRights.title")}</span>
            <p className="text-[12px] text-gray-500">{t("menu.patientRights.desc")}</p>
          </div>
        </button>
        ...
        >
          ...
          <div>
            <span className="text-[15px] font-bold text-gray-900">{t("menu.discrimination.title")}</span>
            <p className="text-[12px] text-gray-500">{t("menu.discrimination.desc")}</p>
          </div>
        </button>
        ...
        >
          ...
          <div>
            <span className="text-[15px] font-bold text-gray-900">{t("menu.contacts.title")}</span>
            <p className="text-[12px] text-gray-500">{t("menu.contacts.desc")}</p>
          </div>
        </button>
      </div>
    </div>
