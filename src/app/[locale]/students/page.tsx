    ...
    <div className="flex min-h-[100dvh] flex-col bg-[var(--color-bg-canvas)]">
      ...
      <main id="main-content" className="flex-1 pb-8">
        <div className="mx-auto max-w-3xl px-4 py-6 md:px-6 md:py-10">
          {/* Editorial hero */}
          <section className="relative mb-8 overflow-hidden rounded-[32px] gradient-brand grain-overlay text-white shadow-4">
            <div
              aria-hidden
            ...
            <div
              aria-hidden
              className="pointer-events-none absolute -bottom-32 -left-24 h-72 w-72 rounded-full opacity-35 blur-3xl"
              style={{ background: "radial-gradient(circle, #A78BFA 0%, transparent 70%)" }}
            />
            <div className="relative grid gap-0 md:grid-cols-[1.1fr_0.9fr]">
              <div className="p-7 md:p-10">
                <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-[11px] font-extrabold uppercase tracking-[0.18em] text-[var(--color-ember-200)] ring-1 ring-[var(--color-ember-400)]/30 backdrop-blur">
                  <Sparkles className="lucide h-3.5 w-3.5" strokeWidth={1.85} />
                  {t("hub.kicker")}
                </span>
                <h1
                  className="mt-4 font-display text-3xl font-extrabold leading-[1.04] tracking-tight md:text-5xl"
                  style={{ letterSpacing: "-0.03em" }}
                >
                  {t("hub.title")}
                </h1>
                <p className="mt-4 max-w-prose text-sm leading-7 text-white/85 md:text-base md:leading-8">
                  {t("hub.subtitle")}
                </p>
                <div className="mt-6 flex flex-wrap gap-2">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1.5 text-[11px] font-bold text-white ring-1 ring-white/15">
                    <Stethoscope className="lucide h-3.5 w-3.5 text-emerald-300" strokeWidth={1.85} />
                    {t("hero.chipFirstAid")}
                  </span>
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1.5 text-[11px] font-bold text-white ring-1 ring-white/15">
                    <BookOpen className="lucide h-3.5 w-3.5 text-[var(--color-ember-300)]" strokeWidth={1.85} />
                    {t("hero.chipLiteracy")}
                  </span>
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1.5 text-[11px] font-bold text-white ring-1 ring-white/15">
                    <Sparkles className="lucide h-3.5 w-3.5 text-indigo-300" strokeWidth={1.85} />
                    {t("hero.chipMissions")}
                  </span>
                </div>
              </div>
